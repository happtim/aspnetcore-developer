...
//为了找到更好的解决方案，您可以使用更高级的搜索策略，称为引导式本地搜索，
//它让求解器可以转义本地最小值 - 该解决方案比附近所有路线都短，但不是全局最小值。偏离本地最小值后，求解器会继续进行搜索。
...
//设置搜索参数
//引导局部搜索（Guided Local Search, GLS） 是一种用于优化组合问题（如车辆路径规划问题，Vehicle Routing Problem，VRP）的元启发式算法。
//它的核心目标是在局部搜索过程中有效地跳出局部最优解，探索更广阔的解空间，从而有可能找到更优的全局解。

//通过在当前解的邻域中寻找更优解来迭代优化。然而，这些算法容易陷入 局部最优，即一种解在其邻域内没有更优解，但并非全局最优。
//GLS 通过动态调整目标函数，引导搜索过程跳出当前的局部最优，继续探索其他可能的解空间区域。
