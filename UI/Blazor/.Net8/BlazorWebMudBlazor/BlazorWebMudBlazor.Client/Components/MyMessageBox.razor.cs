...
#nullable enable
namespace BlazorWebMudBlazor.Client.Components
{
    /// <summary>
    /// A pop-up dialog with a simple message and button choices.
    /// </summary>
    public partial class MyMessageBox : MudComponentBase
    {
        // 添加一个字段来跟踪对话框是否正在显示  
        ...
        public async Task<bool?> ShowAsync(DialogOptions? options = null)
        {
            ...
            try
            {
                ...
                _reference = await DialogService.ShowAsync<MyMessageBox>(title: Title, parameters: parameters, options: options);
                var result = await _reference.Result;

                bool? dialogResult;
                if (result is null || result.Canceled || result.Data is not bool data)
                {
                    dialogResult = null;
                }
                else
                ...
                _currentTaskCompletionSource.SetResult(dialogResult);
                return dialogResult;
            }
            finally
            {
        ...
        public void Close()
        {
            ...
            {
                _currentTaskCompletionSource.SetResult(null);
            }
            _currentTaskCompletionSource = null;
            _reference = null;
        }
